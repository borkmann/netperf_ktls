  ...
  if ((ret = connect(send_socket,
    ...
    if ((SOCKET_EADDRINUSE(ret)) || SOCKET_EADDRNOTAVAIL(ret) || dont_give_up) {
	 ...
	 2008-01-08 */
      ...
      int r1 = setsockopt(send_socket, 6 /* SOL_TCP */, TCP_ULP, "tls", sizeof("tls"));
      int r2 = setsockopt(send_socket, 282 /* SOL_TLS */, TLS_TX, (void *)&tls_tx, sizeof(tls_tx));
      int r3 = setsockopt(send_socket, 282 /* SOL_TLS */, TLS_RX, (void *)&tls_rx, sizeof(tls_rx));

      if (!r1 && !r2 && !r3) {
        printf("INFO: send_socket connected via kTLS!\n");
      } else {
        printf("ERROR: couldn't init kTLS!\n");
        exit(1);
      }
...
recv_omni()
{
  ...
  while ((!times_up) || (units_remaining > 0)) {
	...
	setsockopt(s_listen, SOL_SOCKET, SO_ATTACH_FILTER, &bpf, sizeof(bpf));
	...
	exit(-1);
      ...
      int r1 = setsockopt(data_socket, 6 /* SOL_TCP */, TCP_ULP, "tls", sizeof("tls"));
      int r2 = setsockopt(data_socket, 282 /* SOL_TLS */, TLS_TX, (void *)&tls_tx, sizeof(tls_tx));
      int r3 = setsockopt(data_socket, 282 /* SOL_TLS */, TLS_RX, (void *)&tls_rx, sizeof(tls_rx));

      if (!r1 && !r2 && !r3) {
        printf("INFO: data_socket connected via kTLS!\n");
      } else {
        printf("ERROR: couldn't init kTLS!\n");
        exit(1);
      }
